...
export async function getScheduleState(
  ...
  const [courts, locks, assignments, blocked, forcedData, matchPool] = await Promise.all([
  ...
  const [inPlayPlayerIds, recentlyPlayedSet] = await Promise.all([
    getInPlayPlayerIds(stage),
